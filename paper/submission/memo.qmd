...
**Validation**.
>>>>>>> b1f59995b6a73db19e4d452e703f22b894440517
...
There, we dichotomize the items and take the percentage of respondents giving higher responses, but while that route facilitates plotting by yielding results on the unit interval---but it is not without shortcomings, as seen in the next subsection---this one is better suited to scaling across dozens of items.)
...
This correlation was .9 or higher for twenty-six of our fifty-three items; it was below .7 for just three.
...
```{r dets, fig.align='center', fig.cap="Time Series on Political Interest in Germany", fig.height=4}
...
These means show the dip at the millennium; their correlation with the macrointerest estimates is $r = $`r item_theta_cor %>% filter(item == "int5_allbus") %>% pull(theta_cor) %>% round(2)`.
But this would require adding a second y-axis---the means are about 3 and so do not fit on the unit interval---and, like [Hadley Wickham](https://stackoverflow.com/questions/3099219/ggplot-with-2-y-axes-on-each-side-and-different-scales/3101876#3101876) and others, we prefer to avoid using second axes.
...
This provides a pretty plot, possibly slightly understates the correlation between the ALLBUS and the macrointerest estimates, and avoids prompting unnecessary concern.
...
On this parenthetical point, we now highlight the contextual nature of the theories we address at pages 7-9.)
