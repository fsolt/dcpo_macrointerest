...
Together, the model's difficulty, dispersion, and country-specific bias parameters work to generate comparable estimates of the latent variable of macrointerest from the available but incomparable source data.^[See a discussion of how other data issues, such as sample representation, may affect the estimated outcome in Appendix\nobreakspace{}\@ref(dcpo).]
...
```{r dcpo_summary, include=FALSE, cache=TRUE}
...
In Figure\nobreakspace{}\ref{internal_val}, the macrointerest scores are compared to responses to individual source-data survey items that were used to generate them; this provides an 'internal' convergent validation test [see, e.g., @Caughey2019, p. 686 in which the correlations between the estimated variable and other indicators vary from 0.65 to 0.96.
...
In all three cases, the correlations, estimated taking into account the uncertainty in the measures, are noticeable.
...
# The data is needed for appendix table.
...
Figure\nobreakspace{}\ref{model} displays the results.^[See the trajectories of the all available countries in Appendix \@ref(trajectory).]
...
Beyond investigating the sources of macrointerest, the novel cross-national measurement also provide opportunity for further examining its relationship with various types of political participation and civil engagements [@Anderson2007;@VerbaEtAl1995a].
...
# Full Country Trajectory Matrix of Macrointerest {#trajectory}

```{r trajectory, fig.align='center', fig.height=9, include=FALSE}
df_plot_fullCountry <- mutate(theta_summary, continent = countrycode(country, "country.name", "continent"))

df_plot_fullCountry$continent[df_plot_fullCountry$country %in% c("Kosovo", "Northern Ireland")] <- "Europe"
df_plot_fullCountry$continent[df_plot_fullCountry$continent %in% c("Asia", "Oceania")] <- "Asia-Oceania"

ls_plot_fullCountry <- split(df_plot_fullCountry, df_plot_fullCountry$continent)

plot_fullCountry <- map(ls_plot_fullCountry, \(data){
  ggplot(data, aes(x = year, y = median, group = country)) +
    geom_line() +  # Line for median
    geom_ribbon(aes(ymin = q10, ymax = q90), alpha = 0.2) +  # Shaded area for credible interval
    labs(
    ...
    facet_wrap(~ country, ncol = 4)  # Creates a separate plot for each country
})
...
# Test the Sources of Macrointerest Variances in OECD countries
...
This stability provides a consistent backdrop for analyzing how macrointerests form and evolve over time, minimizing the impact of extreme political or economic upheavals that could skew data in less stable contexts.
...
```{r cpOECD, fig.cap="Data Avaliability: DCPO vs. ESS"}
library(rvest)
tb_ess <-
...
tb_ess_oecd <- filter(tb_ess, country %in% unique(data_combo$country)) |> 
  mutate(across(-country, \(x) ifelse(x == "", 0, 1))) |> 
  pivot_longer(cols = -country, 
               names_to = "year",
               values_to = "avaliable_ess") |> 
  mutate(year = sub("R\\d{1}", "", year) |> as.numeric())

tb_dcpo_oecd <- crossing(country = unique(data_combo$country),
                         year = unique(data_combo$year)) |> 
  mutate(avaliable_dcpo = 1)

tb_oecd <- left_join(tb_dcpo_oecd, tb_ess_oecd) |> 
  mutate(avaliable = rowSums(across(c(avaliable_dcpo, avaliable_ess)), na.rm = TRUE),
         avaliable_fct = factor(avaliable, labels = c("DCPO", "ESS"))) |> 
  select(country, year, avaliable, avaliable_fct)
...
```{r ppc, fig.cap="Posterior Predicted Distributions of Macrointerest Given the Good/Bad Time Indicators. The lines indicates the median of `yrep`. The light ribbon represent the 80 percent central intervals, and the dark one represents the 95 central intervals.", fig.height=9, eval=FALSE}

# the convergence has a low bfmi issue, just for the purpose of illustration

if(file.exists(here("data", "result_ppc.rds"))) {
  ls_plot <- readRDS(here("data", "result_ppc.rds"))
} else {
  m1_illustrate <- brm(
    formula = bf(
      mean * 100 | mi(sd * 100) ~ election +
      ...
      bf(gini_mean | mi(gini_mean_se) ~ 1 + (1 | gini_mean)) +
      bf(gini_diff | mi(gini_diff_se) ~ 1 + (1 | gini_diff)),
  ...
  saveRDS(m1_illustrate, here("data", "result_ill.rds"))
  ...
  ls_plot <- map2(ls_x, ls_x_name, \(x, xName) {
    pp_check(
      m1_illustrate,
      prefix = "ppd",
      type = "ribbon",
  ...
  saveRDS(ls_plot, here("data", "result_ppc.rds"))
}

wrap_plots(ls_plot, ncol = 2)
