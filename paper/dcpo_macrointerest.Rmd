...
In the top left panel of Figure\nobreakspace{}\@ref(fig:item_country_plots), the twelve countries with the most country-year-item observations are displayed.
...
if (!exists("results_path")) {
  ...
  if (!file.exists(file.path(results_path, paste0("dcpo-", str_extract(latest, "\\d{12}"), "-1.csv")))) {
    dir.create(results_path,
               showWarnings = FALSE,
               recursive = TRUE)
    osf_retrieve_node("9uvdk") %>% # needs updated
      osf_ls_files() %>%
...
In Figure\nobreakspace{}\@ref(fig:internalval), the macrointerest scores are compared to responses to individual source-data survey items that were used to generate them; this provides an 'internal' convergent validation test [see, e.g., @Caughey2019, 686].
...
The proportionality of the electoral system is measured using the Gallagher least-squares index of disproportionality, which measures the disparity between parties' vote shares and their seat shares [@Gallagher1991, 40-41; @Gallagher2023].
...
gallegher0 <- map_df(5:48, ~ extract_tables(here("data-raw",
                                                 "ElectionIndices.pdf"),
                                            pages = c(.x))[[1]] %>% 
                       as_tibble())
...
The measurement uncertainty in the data for both macrointerest and income inequality was incorporated into the analysis with the "method of composition," a technique frequently utilized across numerous studies in political science [e.g., @KastellecEtAl2015; @CaugheyWarshaw2018a; @TaiEtAl2022a].^[
See @TaiEtAl2022a [, Supplementary Material C] for a comprehensive review and application of the method of composition in a similar context of cross-national time-series of public opinion.]
...
A tabular version of these results can be found in Appendix\nobreakspace{}\@ref(ESScomparison)].
...
The difference between countries with and without federalism is estimated be (`r get_coef("federal", type = "std_coef")` points, with `round(p_direction(m1, parameters = "federal")[[2]]*100, 1)`% of the posterior distribution greater than zero.
...
Regarding the debate on whether macrointerest is invigorated or instead discouraged by bad times, the evidence of our cross-national analysis regarding economic conditions falls heavily on the side of the latter.
Supporting modernization theory, increases in per capita GDP have a positive short-term effect on aggregate political interest, with a two-standard-deviation increase associated with `r get_coef("gdppc_diff", type = "std_coef")` points more macrointerest and `round(p_direction(m1, parameters = "gdppc_diff")[[2]]*100, 1)`% of the posterior distribution greater than zero.
The long-term, historical effect as evidenced by differences in mean levels across countries is found to be `r get_coef("gdppc_mean", type = "std_coef")` points, albeit with only `round(p_direction(m1, parameters = "gdppc_mean")[[2]]*100, 1)`% of the posterior distribution greater than zero.
A two-standard-deviation increase in the growth in the national economy is estimated to boost macrointerest by `r get_coef("growth_diff", type = "std_coef")` points, with `round(p_direction(m1, parameters = "growth_diff")[[2]]*100, 1)`% of the posterior taking on positive values.
On the other hand, only `round(p_direction(m1, parameters = "growth_mean")[[2]]*100, 1)`% of the posterior distribution for the long-run effect of economic growth is positive, and the findings with regard to unemployment do not provide strong evidence of negative or positive effects either.
...
Year-to-year changes in income inequality are found to make little difference---it would seem that, from one perspective, the influence of the wealthy over the political agenda does not change on such a short time scale, and from the other, there is no evidence that the public reacts to worsening conditions in the distribution of income with greater interest in its agents' actions.
Taken as a whole, this evidence indicates that at least with regard to economic conditions, it is good times, not bad ones, that yield higher macrointerest.
...
And although the single-country study in @Peterson2022 [, 219] concludes that bad times prompt increased macrointerest, this evidence shows the opposite, that it is _good_ economic conditions that lead the public to take interest in politics.
...
These items are listed in Table \@ref(tab:itemTb) below, along with the dispersion ($\alpha$) and difficulty ($\beta$) scores estimated for each from the DCPO model.
...
The survey dataset codes listed in Table \@ref(tab:itemTb) correspond to those used in that package; Table \@ref(tab:surveyInfo) lists these codes with the citation of each dataset.
...
items_summary <- dcpo_input_raw1 %>%
  ...
  group_by(item) %>%
  mutate(
    survey = str_extract(survey, "^[a-z]*"),
    item_survey = str_extract(item, "[a-z]*$"),
    all_surveys = paste0(c(sort(unique(survey %>% str_subset(item_survey))),
                           sort(unique(survey %>% str_subset(item_survey,
                                                             negate = TRUE)))),
                         collapse = ", ")) %>%
  ungroup() %>%
...
```{r cpOECD, fig.cap="Data Availability: DCPO Macrointerest vs. ESS", fig.height=5}
...
# Tabular Version of Results Presented in Figure \@ref(fig:resplot)
...
modelsummary(
  ...
  title = "Tabular Version of Results Presented Graphically in Figure 5",
  booktabs = TRUE
)
```
