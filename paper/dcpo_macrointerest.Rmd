...
dcpo_input_raw <- DCPOtools::dcpo_setup(vars = surveys_interest,
                                        datapath = here("..",
                                                        "data",
                                                        "dcpo_surveys"),
                                        file = here("data",
...
In all, `r n_items` such survey items were identified that were asked in no fewer than five country-years in countries surveyed at least twice; these items were drawn from `r n_surveys` different survey datasets.^[
The complete list of macrointerest survey items is included in online Appendix A.]
...
The Germany, with `r de_obs` observations, is far and away the best represented country in the source data, followed by `r others`.
...
The upper right panel shows the twelve countries with the most years observed; this group is similar to that on the left, but with `r adding` joining the list and `r dropping` dropping off.
The bottom panel counts the countries observed in each year.
Country coverage reached its peak in `r y_peak_year`, when respondents in `r y_peak_nn` countries were asked items about their interest in politics.
The next section describes how all of this sparse and incomparable survey data was used to generate complete, comparable time-series macrointerest scores using a latent variable model.


# Estimating Macrointerest
...
To estimate the public's interest in politics across countries and over time, I employ the latest of these methods that is appropriate for data that is not only incomparable but also sparse, the Dynamic Comparative Public Opinion (DCPO) model elaborated in @Solt2020c.^[
...
For a detailed description of the DCPO model, see Appendix B and @Solt2020c [, 3-8]; here, I focus on how it deals with the principal issues raised by our source data, incomparability and sparsity.
...
To address the sparsity of the source data---the fact that there are gaps in the time series of each country, and even many observed country-years have only one or two observed items---DCPO uses simple local-level dynamic linear models, i.e., random-walk priors, for each country.
...
The model was estimated using the `DCPOtools` package for R [@Solt2019], running four chains for 1,000 iterations each and discarding the first half as warmup, which left us with 2,000 samples.
The $\hat{R}$ diagnostic had a maximum value of 1.01, indicating that the model converged. <!-- Rhat confirmed -->
...
Simply being able to generate estimates of macrointerest or any other latent variable, however, does not automatically mean that they are suitable for use. 
...
In the left panel, macrointerest scores are plotted against the percentage of respondents offering the two most interested responses on the most common question in the source data across all country-years: the European Social Survey's four-point item, "How interested are you in politics?"
...
```{r internalval, fig.height = 4, fig.cap = "Internal Convergent Validation: Correlations Between Macrointerest and Individual Source-Data Survey Items \\label{internal_val}"}
...
```{r extval1, fig.cap="External Convergent Validation: Correlations Between Macrointerest and Unused Political-Interest Survey Items \\label{external_val}", fig.height=4}
...
And the relative power theory holds that greater income inequality, by increasingly concentrating political power in the hands of the wealthy, shapes the political agenda in ways that discourage the broader public from taking interest [see, e.g., @Solt2008].
In each of these circumstances, macrointerest is argued to grow in _good_ times and wither as the context worsens.

Data to test these hypotheses regarding the causes of macrointerest are drawn from several sources.
The Democratic Electoral Systems (DES) dataset updated in @Bormann2022 provides information about the timing of elections, a dichotomous variable coded one in election years and zero when no election was held.
...
Federalism is a third dichotomous variable, coded one in countries with strong federal systems and zero in all others [see @Lijphart1999].
...
The resulting dataset comprises the thirty-seven OECD democracies, each observed in eleven (Colombia) to forty (Ireland and the United States) consecutive years (mean: 30.86 years, median: 31 years).
...
The former help account for heteroskedasticity across space due to, e.g., omitted variable bias while permitting the inclusion of time-invariant predictors such as, in this dataset, parliamentarism and federalism, and the latter take into account 'time shocks' that operate on all countries simultaneously [@Shor2007, 171-172].
Further, the 'within-between random effects' specification is employed, meaning each of the time-varying predictors are decomposed into its time-invariant country mean and the difference between each the country-year value and this country mean; this specification has been shown superior to fixed effects and other commonly used TSCS specifications for addressing omitted variable bias and endogeneity [see @Bell2015].
...
```{r swiid_data}
if (!file.exists(here("data-raw" ,"swiid9_4/swiid9_4_summary.csv"))) {
...
swiid_summary <- read_csv("data-raw/swiid9_4/swiid9_4_summary.csv") %>% 
  mutate(country = countrycode::countrycode(country,
                                            "country.name",
                                            "country.name")) %>% 
...
if (!file.exists(here::here("data", "results.rda"))) {
  ...
  save(doubled_sd, coef_data0, file = here::here("data", "results.rda"))
} else {
  load(file = here::here("data", "results.rda"))
}
```

```{r resplot, fig.cap="Predicting Macrointerest in OECD Democracies \\label{model}", fig.height = 6, fig.width = 7.5}
...
coef_data0 %>% 
  ...
  xlab(NULL) +
  ylab(NULL) +
  plot_annotation(caption = "Notes: Dots indicate posterior means; whiskers, from thickest to thinnest, describe 80%, 90%, and 95% credible intervals; shading depicts the posterior probability density function.")
```
...
The difference between countries with and without federalism is estimated to be even larger, `r get_coef("federal")` points, though only its 90% credible interval (`r get_coef("federal", type = "ci", width = "0.9")`) is bounded away from zero.
And although disproportionality is not estimated to have long-run effects that consistently distinguish countries with more or less proportional electoral results, _changes_ in disproportionality appear to have an immediate effect: a two-standard-deviation increase in the Gallagher index yields a `r get_coef("lsq_diff")` in the posterior mean.

On the debate on whether macrointerest is invigorated by bad times or instead is discouraged, the evidence from this cross-national analysis falls heavily on the side of the latter.
...
The long-term effect evidenced by differences in mean levels across countries is found to be larger, `r get_coef("gdppc_mean")`, but only its 80% credible interval excludes zero.
The posterior means for growth in the national economy are positive as well, but despite this topic's frequent coverage in media, the posterior distributions do not suggest the relationships are statistically defensible.
The findings with regard to unemployment, another circumstance typically closely followed by the media, are similar.
...
# Conclusions
"the electorate becomes more interested and attentive to government when times are bad" [@Peterson2022, 219]
...
@Solt2020c demonstrates that the DCPO model provides a better fit to survey data than the models put forward by @Claassen2019 or @Caughey2019.
The @McGann2019 model depends on dense survey data unlike the sparse data on political interest described in the preceding section.
@Kolczynska2020 is the very most recent of these five works and builds on each of the others, but the MRP approach developed in that piece is suitable not only when the available survey data are dense but also when ancillary data on population characteristics are available, so it is similarly inappropriate to this application.]
The DCPO model is a population-level two-parameter ordinal logistic item response theory (IRT) model with country-specific item-bias terms.
