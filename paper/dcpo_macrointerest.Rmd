...
The current version of the software facilitates the entire practical data generation process: from accessing the original survey datasets and converting them into R standard formats for quick loading; through standardizing country names, identifying survey years, and extracting the desired survey items; to restructuring the resulting data for analysis with the DCPO model. 
...
```{r surveyInfo}
tb_survey <- read_csv(here("data-raw", 
                           "survey_info_macrointerest.csv"),
                      col_types = "cccccc") |>
  mutate(survey_code = str_remove(survey, "\\d+") |>
           str_remove(",.*") |>
           str_remove("_.*"),
         creator = ifelse(
           is.na(creator),
         ...
         version = paste(survey,
                         ifelse(is.na(version),
                                "",
                                str_to_lower(version)))) |>
  filter(!is.na(name)) |> 
...
tb_survey %>% 
  transmute(`Survey Dataset Code` = survey_code,
            `Survey Name` = name,
            `Datasets and Versions*` = version,
            `Creator(s)` = creator) %>% 
  ...
  footnote(symbol = "Survey dataset codes correspond to those used in the DCPOtools R package (Solt, Hu, and Tai 2019).")
