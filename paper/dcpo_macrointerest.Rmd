...
In light of the growing threats to democracy seen in many countries, measuring the levels and trends of aggregate political interest---macrointerest---and understanding their sources is therefore crucially important [@Peterson2023].
Data to allow such assessments, however, have been unavailable.
Although many surveys ask respondents across countries how interested they are in politics, differences in question wording and in response categories have limited scholars' ability to pool the data together, and even in the absence of these issues, in most cases, the questions have not been asked sufficiently frequently to provide annual time series.
...
It shows that these cross-national macrointerest scores perform impressively well in validation tests.
...
The results support arguments that, in these countries, election campaigns, inclusive institutions, and good times, not bad ones, spur greater political interest.
...
In all, `r n_items` such survey items were asked in no fewer than five country-years in countries surveyed at least twice; these items were drawn from `r n_surveys` different survey datasets.^[
...
Germany, with `r de_obs` observations, is far and away the best represented country in the source data, followed by `r others`.
...
The next section describes how all of this sparse and incomparable survey data was used with a latent variable model to generate complete, comparable time-series macrointerest scores.
...
For a detailed description of the DCPO model, see Appendix B and @Solt2020c [, 3-8]; the focus here is on how it deals with the principal issues raised by the source data, incomparability and sparsity.
...
```{r extval1, fig.cap="Construct Validation: Correlations Between Macrointerest and Other Aspects of Political Engagement \\label{external_val}", fig.height=4}
