...
```{r disp}
if (!file.exists(here::here("data-raw", "gallegher_data.rda"))) {
  ...
  rio::export(oecd, here::here("data-raw", "gallegher_data.rda"))
} else {
  gallegher <- rio::import(here::here("data-raw", "gallegher_data.rda"))
}    
```
...
The difference between countries with and without federalism is estimated be `r get_coef("federal", type = "std_coef")` points, with `r round(p_direction(m1, parameters = "federal")[[2]]*100, 1)`% of the posterior distribution greater than zero.
...
Regarding the debate on whether macrointerest is invigorated or instead discouraged by bad times, the evidence of our cross-national analysis the impact of economic conditions falls heavily on the side of the latter.
...
The long-term, historical effect as evidenced by differences in mean levels across countries is found to be `r get_coef("gdppc_mean", type = "std_coef")` points, albeit with only `r round(p_direction(m1, parameters = "gdppc_mean")[[2]]*100, 1)`% of the posterior distribution greater than zero.
...
And although the single-country study in @Peterson2022 [, 219] concludes that bad times prompt increased macrointerest, this evidence shows the opposite, that with regard to economic conditions it is _good_ times that lead the public to take interest in politics.
...
The survey dataset codes listed in Table \@ref(tab:itemsTb) correspond to those used in that package; Table \@ref(tab:surveyInfo) lists these codes with the citation of each dataset.
...
plot_comparison <- ggplot(data = df_compare,
  ...
  labs(y = "Macrointerest", title = "Wcalc vs. DCPO in the United States") +
