...
In light of the growing threats to democracy seen in many countries, measuring the levels and trends of political interest and understanding their sources is therefore crucially important [@Peterson2023].
Such data, however, are scarce.
...
First, it incorporates the _difficulty_ of each question's responses, that is, how much interest in politics is indicated by a given response. To say that one is "very interested" in politics, for example, is to exhibit more interest than to say that one is "somewhat interested" or "not very interested."
...
That estimates can be generated for macrointerest or any other latent variable, however, does not automatically mean that they are suitable for use. 
...
```{r extval1, fig.cap="Construct Validation: Correlations Between Macrointerest and  \\label{external_val}", fig.height=4}
...
```{r resplot, fig.cap="Predicting Macrointerest in OECD Democracies \\label{model}", fig.height = 4.5, fig.width = 7.5}
...
The long-term, historical effect evidenced by differences in mean levels across countries is found to be larger, `r get_coef("gdppc_mean")`, but only its 80% credible interval excludes zero.
The posterior means for growth in the national economy are positive as well, but despite this topic's frequent coverage in media, the posterior distributions do not suggest relationships that are statistically defensible.
